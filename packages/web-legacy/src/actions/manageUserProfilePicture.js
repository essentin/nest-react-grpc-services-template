...
export function updateStepThree(isPhotoSkipped) {
  return async (dispatch, getState, { client }) => {
    try {
      let mutation = gql`
      mutation UserUpdate(
        $isPhotoSkipped: Boolean,
        $stepThree: Boolean
      ){
        userUpdate(
          isPhotoSkipped: $isPhotoSkipped
          stepThree: $stepThree
      ) {
          status
          errorMessage
      ...
      const { data } = await client.mutate({
        mutation,
        variables: { isPhotoSkipped, stepThree: true }
      });

      if (data && data.userUpdate && data.userUpdate.status === 200) {
