...
export function createListPhotos(listId, name, type) {
  return async (dispatch, getState, { client }) => {
    ...
    try {
      ...
      if (
        data &&
        data.CreateListPhotos &&
        data.CreateListPhotos.status === 'success'
      ) {
  ...
  return async (dispatch, getState, { client }) => {
    ...
    try {
      ...
      if (
        ...
        reservationCountData.data.getUpcomingBookings &&
        reservationCountData.data.getUpcomingBookings.count > 0
      ) {
        ...
        if (
          showListPhotosData &&
          showListPhotosData.data.ShowListPhotos &&
          showListPhotosData.data.ShowListPhotos.length <= 1
        ) {
          // If length less
          toastr.error(
            'Error Occured!',
          ...
          if (
            data &&
            data.RemoveListPhotos &&
            data.RemoveListPhotos.status === 'success'
          ) {
