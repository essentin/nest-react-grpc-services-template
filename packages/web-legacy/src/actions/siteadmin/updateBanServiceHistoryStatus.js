...
const mutation = gql`
  ...
  `;
export function updateBanServiceHistoryStatus(id, banStatus, userMail, userName, currentPage, searchList, userType) {
  ...
  return async (dispatch, getState, { client }) => {
    try {
      const { data } = await client.mutate({
        ...
        refetchQueries: [{ query, variables: { currentPage, searchList, userType } }]
      });
