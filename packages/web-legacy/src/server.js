...
import passport from './core/auth/passport/passport';
...
import facebookAuth from './core/auth/routes/facebook';
import googleAuth from './core/auth/routes/google';
import linkedinAuth from './core/auth/routes/linkedin';
...
import listSettingsIconPhotoUpload from './core/listSettingsIconPhotoUpload';
...
import capturePaymentIntents from './core/cron/capturePaymentIntents';
...
import cancellationUpload from './core/cancellationUpload';
...
listSettingsIconPhotoUpload(app);

cancellationUpload(app);
...
app.get('*', async (req, res, next) => {
  try {
    ...
    let account = store.getState().account && store.getState().account.data;
    let featureFlag = await getFeatureFlag(
      req.user && req.user.id,
      account && account.email,
    );
    await store.dispatch(setFeatureFlag(featureFlag || {}));
    ...
    let collectionArray = ['/users/bookings/itinerary/', '/review/write/'];
    ...
    if (req.user && !req.user.admin) {
      if (account && !account.stepTwo && req.path !== '/register/step-two')
        return res.redirect('/register/step-two');
      if (
        account &&
        account.stepTwo &&
        !account.stepThree &&
        req.path !== '/register/step-three'
      )
        return res.redirect('/register/step-three');
    }
