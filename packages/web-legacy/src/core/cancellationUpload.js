import { cancellationDir } from '../config';
    ...
    async (req, res, next) => {
      ...
      const smallImage = await new Promise((resolve, reject) => {
        sharp(file.path)
          ...
          .toFile(cancellationDir + 'small_' + file.filename, function(err) {
            if (file) {
              resolve(file);
            } else {
              reject(error);
      ...
      if (file.mimetype === 'image/jpeg') {
        ...
        const mediumImage = await new Promise((resolve, reject) => {
          sharp(file.path)
            ...
            .jpeg({ quality: 50 })
            //.crop(sharp.strategy.entropy)
            .toFile(cancellationDir + 'medium_' + file.filename, function(err) {
              if (file) {
                resolve(file);
              } else {
                reject(error);
...
export default cancellationUpload;
