...
import { fileuploadDir, profilePhotouploadDir } from '../config';
import multer from 'multer';
import sharp from 'sharp';
import { Listing, ListPhotos, Reservation, User, UserProfile, WishList } from '../data/models';
      ...
      try {
        ...
        if (
          ...
          if (requestData.to && requestData.type) {
            sendEmailStatus = await sendEmail(
              requestData.to,
              requestData.type,
              requestData.content,
            );
            ...
            if (isListingAvailable) {
              ...
              if (getReservationCount > 0) {
                const listPhotosCount = await ListPhotos.count({
                  where: { listId },
                  include: [{ model: Listing, as: 'listing', where: where }],
                });
          ...
          details: [
            {
              ...
              paths: ['/password/verification/', '/security'],
            },
          ],
        },
      };
