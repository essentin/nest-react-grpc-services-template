  ...
  new FacebookStrategy(
    ...
    (req, accessToken, refreshToken, profile, done) => {
      ...
      const facebookLogin = async () => {
        if (req.user) {
        ...
        } else if (profile._json && profile._json.email && !profile._json.email.match(/\@/g)) { // checking email in facebook
          done(null, { type: 'emailRequired' });
        } else {
          let userType = Number(req.cookies.userType);
          // Check if the email is already available
          const userLogin = await User.findOne({
            attributes: ['email', 'id', 'userBanStatus', 'userDeletedAt', 'userType'],
          ...
          if (userLogin) {
            ...
            let email = profile && profile._json && profile._json.email;
            let inviteCode = req.cookies.inviteCode;

            let isInvited = await isUserInvited(email, inviteCode);
            ...
            if (
              ...
              profile._json.birthday != null &&
              profile._json.birthday != undefined
            ) {
            ...
            let updatedLastName = capitalizeFirstLetter(
