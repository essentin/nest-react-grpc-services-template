    ...
    async (req, res, next) => {
      ...
      const xSmallImage = await new Promise((resolve, reject) => {
        sharp(files[0].path)
          ...
          .resize(300, null)
          //.crop(sharp.strategy.entropy)
          .toFile(fileuploadDir + 'x_small_' + files[0].filename, function(
            err,
          ) {
            if (files) {
              resolve(files);
      ...
      if (files[0].mimetype === 'image/jpeg') {
        ...
        const mediumImage = await new Promise((resolve, reject) => {
          sharp(files[0].path)
            ...
            .jpeg({ quality: 80 })
            //.crop(sharp.strategy.entropy)
            .toFile(fileuploadDir + 'x_medium_' + files[0].filename, function(
              err,
            ) {
              if (files) {
                resolve(files);
        ...
        const largeImage = await new Promise((resolve, reject) => {
          sharp(files[0].path)
            ...
            .jpeg({ quality: 80 })
            //.crop(sharp.strategy.entropy)
            .toFile(fileuploadDir + 'x_large_' + files[0].filename, function(
              err,
            ) {
              if (files) {
                resolve(files);
        ...
        const largeImageSize = await new Promise((resolve, reject) => {
          sharp(files[0].path)
            ...
            .jpeg({ quality: 80 })
            //.crop(sharp.strategy.entropy)
            .toFile(fileuploadDir + 'xx_large_' + files[0].filename, function(
              err,
            ) {
              if (files) {
                resolve(files);
        ...
        const placeholderImage = await new Promise((resolve, reject) => {
          sharp(files[0].path)
            ...
            //.crop(sharp.strategy.entropy)
            .toFile(
              fileuploadDir + 'placeholder_' + files[0].filename,
              function(err) {
                if (files) {
                  resolve(files);
                } else {
