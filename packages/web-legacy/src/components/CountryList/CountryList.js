...
class CountryList extends Component {
    ...
    render() {
        const { data: { loading, getCountries }, className, input, isEmptyFirst,
            dialCode, onChange, isFrom } = this.props;
        const { formatMessage } = this.props.intl;
        return (
            <div>
                <FormControl componentClass="select"
                    className={className}
                    disabled={isFrom ? true : false}
                    {...input}
                    onChange={(e) => {
                        input.onChange(e);
                ...
                >
                    ...
                    {
                        !loading && !isFrom && getCountries != null && getCountries.length > 0 && !dialCode && getCountries.map((item) => {
                            return (
...

                    {
                        !loading && isFrom && getCountries != null && getCountries.length > 0 && !dialCode && getCountries.map((item) => {
                            return (
                                <option value={'SE'} key={item.id}>{'Sweden'}</option>
                            )
                        })
                    }
