...
class EditListSettingsForm extends Component {
  ...
  componentWillMount() {
    ...
    if (activityTypeList != undefined && activityTypeList.data && activityTypeList.data.results.length > 0) {
      this.setState({
        activityTypeData: activityTypeList.data.results,
      });
    }
  }
  ...
  render() {
    const { error, handleSubmit, submitting, dispatch, itemDescription } = this.props;
    const { formatMessage } = this.props.intl;
    const { id, typeId, deleteListSettings, thumbnail, isEnable } = this.props;
    const { fieldType, activityTypeData } = this.state;
    return (
