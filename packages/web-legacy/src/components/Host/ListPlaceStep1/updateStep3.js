...
async function updateStep3(values, dispatch) {
  ...
  let activity = values.activity && values.activity.length > 0 ? JSON.stringify(values.activity) : JSON.stringify([]);
  let spaceAvailability = values.spaceAvailability && values.spaceAvailability.length > 0 ? JSON.stringify(values.spaceAvailability) : JSON.stringify([]);
  let spaceOpeningTime = values.spaceOpeningTime && values.spaceOpeningTime.length > 0 ? JSON.stringify(values.spaceOpeningTime) : JSON.stringify([]);
  ...
  let variables = Object.assign({}, {
    ...
    cancellationPolicy, activity, spaceAvailability,
    spaceOpeningTime
  });
  ...
  const query = `query (
    ...
    $spaceOpeningTime: String
      ...
      updateListingStep3 (
        ...
        spaceOpeningTime: $spaceOpeningTime
      ) {
        status
      }
    }`;
