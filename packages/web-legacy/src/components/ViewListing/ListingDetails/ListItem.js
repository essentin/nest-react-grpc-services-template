...
import DotImage from '../../../../public/NewIcon/circleDot.svg'
...
class ListItem extends React.Component {
  ...
  render() {
    const { itemList, label, showLabel, hideLabel, hideImg } = this.props;
      ...
      <div className={cx(s.horizontalLineThrough)}>
        <Row>
          <Col xs={12} sm={12} md={12} lg={12}>
            <p className={cx(s.textMutedNew, s.space2)}> {label} </p>
          </Col>
          <Col xs={12} sm={12} md={12} lg={12}>
            <Row>
              <Col md={12} lg={12}>
                  ...
                  firstArray.map((item, key) => {
                    if (item.listsettings.isEnable === "1") {
                      return (
                        <div
                          key={key}
                          className={cx(s.splitList, s.vtrTop, s.space1, s.splitListWidth, s.displayTableList)}
                        >
                          <div className={s.displayTableRowList}>
                            {!hideImg && <span className={s.listIcon}>
                              {
                                item.listsettings.itemDescription ?
                                  <img src={'/images/amenities/' + item.listsettings.itemDescription} className={s.imgSection} /> :
                                  <img src={NoLogo} className={s.imgSection} />
                              }
                            </span>}
                            {
                              hideImg && <span className={s.dotImageScetion}>
                                <img src={DotImage} className={s.dotImage} />
                              </span>
                            }
